...
export const Swimlane = props => (
  <div className={ 'swimlane__container' }>
    { props.lanes.map(lane => (<Lane key={lane.title} title={ lane.title } start={ props.start } end={ props.end } cards={ lane.cards } />)) }
  </div>
)
...
export const LaneDetail = props => (
  <div className={ 'swimlane__detail' }>
    { props.cards.map(card => (<Card  key={card.description} laneStart={ props.start } laneEnd={ props.end } className={ card.className } style={ card.style } description={ card.description } start={ card.start } end={ card.end } />)) }
  </div>
)
